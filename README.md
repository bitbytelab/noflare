...
- `MAX_WORKERS` - Max concurrent Chromium tabs (default: `4`)
- `HEADLESS` - `true`/`false` to run Chromium headless (default: `false`)
- `SANDBOX` - `true`/`false` to run Chromium in Sandbox (default: `false` in docker)
- `DATA_DIR` - Path to persistent user-data directory (default: `~/.noflare/data`)
- `DISABLE_MEDIA` - Disable loading (Images/Media) (default: `false`)
- `LOCALE` - Browser language/locale (default: `en-US`)
